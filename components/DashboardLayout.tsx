    ...
    <View className="flex flex-row bg-[#F7F7F7]">
      ...
      >
        <View className="flex-row justify-between">
          <View className="sm:mb-3 mb-0">
            {/* <Logo hasMargin={false} /> */}
            <Text>Here we are</Text>
          </View>
          <TouchableOpacity
            onPress={() => setIsSidebarOpen(false)}
            className="md:hidden bg-[#283047] text-[#F9FFF7] text-base font-bold z-10 flex items-center justify-center rounded-full w-8 h-8"
          >
            <Text>✕</Text>
          </TouchableOpacity>
        </View>

        <View className="flex flex-row justify-between items-center  mt-10 px-5">
          <View className="">
      ...
      <View style={tw`flex-1 flex-col mt-6 mx-5`}>
        ...
        {/* Content */}
        <View className="flex-1 bg-[#F7F7F7] mx-5 mt-6">{children}</View>
      </View>
    </View>
  );
}
